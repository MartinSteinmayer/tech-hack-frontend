...
export async function getAiRecommendations(description, criteria = [], category = '', mockSuppliers = []) {
    try {
        ...
        // This is the key fix - we're sending supplier data in the request
        const supplierData = mockSuppliers.map(supplier => ({
            ...
            // Add relevant attributes based on criteria
            rating: Math.random() * 5, // Mock rating between 0-5
            sustainabilityScore: criteria.includes('sustainable') ? Math.random() * 100 : Math.random() * 50,
            pricing: criteria.includes('premium') ? 'Premium' :
                criteria.includes('budget') ? 'Budget' : 'Standard',
            deliverySpeed: criteria.includes('fast') ? 'Fast' : 'Standard',
            isoCertified: criteria.includes('certified'),
            isLocal: criteria.includes('local'),
        ...
        const requestOptions = {
            ...
            body: JSON.stringify({
                ...
                criteria: criteria,
                suppliers: supplierData // This is the key fix
            })
        };

        // Make the API call
        const response = await fetch('https://tech-hack-api.vercel.app/api/suppliers/recommend', requestOptions);
